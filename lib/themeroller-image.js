...
function processImageMagick( args, callback ) {
	...
	// Note: Update `convert` to `magick` for ImageMagick 7+
	// when ready; see the comment at `.github/workflows/node.js.yml`.
	// When updating here, replace other occurrences of `convert`, too.
	...
	proc.on( "close", code => {
		if ( code !== 0 ) {
			return callback( new Error( `convert process exited with code ${ code }: ${ Buffer.concat( stderrBuffers ).toString() }` ) );
		}
...
generateIcon = function( params, callback ) {
	...
	// Full command:
	// $ convert <icons_mask_filename> -set colorspace RGB -background <color> -alpha shape -channel A -gamma 0.5 -set colorspace sRGB output.png
