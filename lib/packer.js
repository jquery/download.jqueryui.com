	...
	pack: function( callback ) {
		...
		function pack( callback ) {
			...
			if ( !theme.isNull ) {
				...
				if ( semver.gte( build.pkg.version, "1.11.0pre" ) ) {
					add( "development-bundle/themes/" + theme.folderName() + "/theme.css", theme.css() );
					add( "development-bundle/themes/" + theme.folderName() + "/minified/theme.min.css", banner( build.pkg, null, { minify: true } ) + sqwish.minify( util.stripBanner({ data: theme.css() }) ) );
				} else {
