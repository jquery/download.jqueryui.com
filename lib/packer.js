...
Packer.prototype = {
	...
	pack: function( callback ) {
		...
		build.componentMinFiles.into( "development-bundle/ui/minified", /^ui/ ).forEach( add );

		// Base theme (baseThemeFiles have been included by componentFiles)
		build.baseThemeMinFiles.into( "development-bundle/themes/base/minified", /^themes\/base/ ).forEach( add );
		build.baseThemeImages.into( "development-bundle/themes/base/minified/images", /^themes\/base\/images/).forEach( add );
		...
		build.bundleJs.into( "jquery-ui" + options.bundleSuffix, /^jquery-ui/ ).into( [ { regExp: /^jquery-ui/, newSubStr: "js/jquery-ui-" + build.pkg.version }, "development-bundle/ui/" ] ).forEach( add );
		build.bundleJsMin.into( "jquery-ui" + options.bundleSuffix, /^jquery-ui/ ).into( [ { regExp: /^jquery-ui/, newSubStr: "js/jquery-ui-" + build.pkg.version }, "development-bundle/ui/minified/" ] ).forEach( add );
		...
		if ( !this.options.skipTheme ) {
			build.bundleCss( theme ).into( [ { regExp: /^jquery-ui/, newSubStr: "css/" + theme.folderName() + "/jquery-ui-" + build.pkg.version + options.bundleSuffix }, "development-bundle/themes/" + theme.folderName() + "/" ] ).forEach( add );
			build.bundleCssMin( theme ).into( [ { regExp: /^jquery-ui/, newSubStr: "css/" + theme.folderName() + "/jquery-ui-" + build.pkg.version + options.bundleSuffix }, "development-bundle/themes/" + theme.folderName() + "/minified/" ] ).forEach( add );
		}
		...
		if ( !this.options.skipTheme ) {
			...
			}).into( "development-bundle/themes/" + theme.folderName() + "/minified", /^themes\/base/ ).forEach( add );
		}
		if ( !theme.isNull ) {
		...
		if ( theme.isNull ) {
			...
			async.series([
				function( callback ) {
					var themeImages = JqueryUi.files();
