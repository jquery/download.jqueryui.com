...
function JqueryUiFiles_1_9_0( jqueryUi ) {
	...
	this.demoFiles = Files( new Glob( jqueryUi.path + "demos/*/**", { sync: true } ).found.filter( noDirectory ).map( stripJqueryUiPath ).map( readFile ) );

	this.docFiles = Files( new Glob( jqueryUi.path + "docs/*", { sync: true } ).found.map( stripJqueryUiPath ).map( readFile ) );
  ...
  sync: true } ).found.filter( noDirectory ).map( stripJqueryUiPath ).map( readFile ) );
	...
	this.jqueryCore = Files( new Glob( jqueryUi.path + "jquery-*", { sync: true } ).found.map( stripJqueryUiPath ).map( readFile ) );
