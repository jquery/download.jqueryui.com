...
files.prototype.into = function( newSubStrs, regExp ) {
	regExp = regExp || /^/;
	return this.map(function( file ) {
		return util.alwaysArray( newSubStrs ).map(function( newSubStr ) {
			if ( typeof newSubStr === "string" ) {
				return { path: file.path.replace( regExp, newSubStr ), data: file.data };
			} else if ( typeof newSubStr === "object" ) {
				return { path: file.path.replace( newSubStr.regExp, newSubStr.newSubStr ), data: file.data };
			} else {
				throw new Error( "invalid format of " + newSubStr );
			}
		});
