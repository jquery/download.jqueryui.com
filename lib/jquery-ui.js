var commonFiles, componentFiles, testFiles, virtualFiles,
...
function JqueryUiFiles( jqueryUi ) {
	var self = this,
		...
		readFile = function( path ) {
			...
			if ( (/(js|css)$/).test( path ) ) {
				data = replaceVersion( data.toString( "utf-8" ) );
			}
			return data;
		},
	...
	Object.keys( virtualFiles ).forEach(function( group ) {
		Object.keys( virtualFiles[ group ] ).forEach(function( src ) {
			...
			if ( typeof dest === "string" ) {
				...
				glob( jqueryUi.path + src ).filter( noDirectory ).map( stripJqueryUiPath ).forEach(function( src ) {
