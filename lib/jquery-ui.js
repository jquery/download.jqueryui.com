...
function JqueryUiFiles( jqueryUi ) {
	var self = this,
		cacheIt = function( srcpath ) {
			var destpath = srcpath;
			filesCache[ jqueryUi.path ][ destpath ] = readFile( jqueryUi.path + srcpath );
		},
		path = require( "path" ),
	...
	this.data = filesCache[ jqueryUi.path ] = {};
	this.data.minified = {};

	this.commonFiles = commonFiles.map(function( path ) {
		return glob( jqueryUi.path + path ).filter( noDirectory ).map( stripJqueryUiPath );
	}).reduce( flatten, [] );
	...
	this.demoFiles = glob( jqueryUi.path + "demos/*/**" ).filter( noDirectory ).map( stripJqueryUiPath );

	this.docFiles = glob( jqueryUi.path + "docs/*" ).map( stripJqueryUiPath );
	...
	this.testFiles.forEach( cacheIt );
	...
	this.i18nFiles = glob( jqueryUi.path + "ui/i18n/*" ).map( stripJqueryUiPath );

	this.jqueryFilename = stripJqueryUiPath( glob( jqueryUi.path + "jquery-*" )[ 0 ] );
	this.themeFiles = glob( jqueryUi.path + "themes/base/**" ).filter( noDirectory ).map( stripJqueryUiPath );
}

JqueryUiFiles.prototype = {
	min: function( file ) {
		var minified = this.data.minified;

		if ( !minified[ file ] ) {
			if ( (/.js$/i).test( file ) ) {
				minified[ file ] = UglifyJS.minify( this.data[ file ].toString( "utf8" ), { fromString: true } ).code;
			} else if ( (/.css$/i).test( file ) ) {
				minified[ file ] = sqwish.minify( this.data[ file ].toString( "utf8" ) );
			}
		}
