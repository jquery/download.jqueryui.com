...
function Package() {
	...
	this.jsFileNames = this.runtime.components.map(function( component ) {
		return component + ".js";
	});
	this.pkgJson = JSON.parse( this.files[ "package.json" ] );
...
extend( Package.prototype, {
	...
	"jquery-ui.structure.css": function( callback ) {
		...
		jqueryCssBuilder( this.runtime.files, "structure", {
			appDir: "ui",
			include: this.runtime.components
		}, function( error, css ) {
			if ( error ) {
			...
			themeCss.promise.then(function() {
				var banner = self.baseThemeCss.data
					.replace( /\*\/[\s\S]*/, "*/" )
					.replace( /\n.*\n.*themeroller.*/, "" );
	...
	"jquery-ui.theme.css": function( callback ) {
		var self = this,
			pkgJson = this.pkgJson,
			themeCss = this.themeCss,
			themeVars = this.runtime.themeVars;

		jqueryCssBuilder( this.runtime.files, "theme", {
			appDir: "ui",
			include: this.runtime.components
		}, function( error, baseThemeCss ) {
			var css, themeroller;

			if ( error ) {
				themeCss.reject( error );
				return callback( error );
			}

			if ( baseThemeCss.length === 0 ) {
				themeCss.resolve( null );
				return callback( null, null );
			}


			self.baseThemeCss = baseThemeCss;
			self.themeroller = themeroller = ThemeRoller( baseThemeCss, themeVars );
			css = themeroller.css();
	...
	"jquery-ui.min.css": function( callback ) {
		...
		Q.all([ this.structureMinCss.promise, this.themeMinCss.promise ]).spread(
			...
			var _banner = banner( pkgJson, cssFileNames, { minify: true } );
			themeMinCss = themeMinCss || "";
			callback( _banner + structureMinCss + themeMinCss );
		}).catch( callback );
	},
