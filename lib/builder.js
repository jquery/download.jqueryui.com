...
Builder.prototype = {
	...
	build: function( callback ) {
		...
		// Minified files
		files.componentFiles.filter( selected ).forEach(function( filepath ) {
			var data = files.min( filepath );
			if ( (/^ui\//).test( filepath ) ) {
				filepath = filepath.replace( /^ui/, "ui/minified" );
			} else if ( (/^themes\//).test( filepath ) ) {
				filepath = filepath.replace( /^themes\/base/, "themes/base/minified" );
			} else {
				return;
			}
			filepath = filepath.replace( /\.([^.]*)$/, ".min.$1" );
			add( null, [ "development-bundle", filepath ], data );
		});
		files.commonFiles.forEach(function( filepath ) {
			if ( (/themes\/base\/images/).test( filepath ) ) {
				add( null, [ "development-bundle/themes/base/minified/images", path.basename( filepath ) ], files.data[ filepath ] );
			}
		});
		add( null, [ "development-bundle/themes/base/minified/jquery.ui.theme.min.css" ], banner( jqueryUi.pkg, null, { minify: true } ) + baseCssMin );
		...
		if ( !this.options.skipTheme ) {
			...
			files.themeFiles.filter( selected ).forEach(function( filepath ) {
				var data = files.min( filepath );
				filepath = filepath.replace( /^themes\/base/, "development-bundle/themes/" + theme.folderName() + "/minified" ).replace( /\.([^.]*)$/, ".min.$1" );
				if ( options.scope ) {
					add( null, [ filepath ], util.scope( data, options.scope ) );
				} else {
					add( null, [ filepath ], data );
				}
			});
		}
		if ( !theme.isNull ) {
		...
		if ( components.indexOf( "datepicker" ) >= 0 ) {
			if ( !files.data[ "ui/i18n/jquery-ui-i18n.js" ] ) {
				files.data[ "ui/i18n/jquery-ui-i18n.js" ] = files.i18nFiles.reduce(function( sum, filepath ) {
					return sum + stripBanner( files.data[ filepath ], filepath );
				}, banner( jqueryUi.pkg, files.i18nFiles.map( path.basename ) ) );
			}
			[ "ui/i18n/jquery-ui-i18n.js" ].forEach( addEach );
			add( null, [ "development-bundle/ui/minified/i18n/jquery-ui-i18n.min.js" ], banner( jqueryUi.pkg, files.i18nFiles.map( path.basename ), { minify: true }) + stripBanner( files.min( "ui/i18n/jquery-ui-i18n.js" ) ) );
		}
