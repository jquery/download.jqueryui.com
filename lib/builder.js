...
// Demo files
demoFiles = glob( release.path + "/demos/*/**" ).filter( noDirectory ).map( stripReleasePath );

// Doc files
docFiles = glob( release.path + "/docs/*" ).map( stripReleasePath );
...
// Auxiliary variables
imageFiles = glob( release.path + "/themes/base/images/*" ).map( stripReleasePath );
jqueryFilename = stripReleasePath( glob( release.path + "/jquery-*" )[ 0 ] );
