...
Builder.prototype = {
	...
	build: function( callback ) {
		...
		build.push({
			...
			data: bundleCss( baseCss, baseTheme )
		});
		...
		build.push({
			...
			data: bundleCss( baseCssMin, baseThemeMin, { minify: true } )
		});
		if ( !this.options.skipTheme ) {
			[ "css/" + theme.folderName() + "/jquery-ui-" + jqueryUi.pkg.version + options.bundleSuffix + ".min.css",
				...
				build.push({
					path: [ basedir, dst ].join( "/" ),
					data: bundleCss( baseCssMin, theme, { minify: true } )
				});
			});
		}
		...
		if ( !theme.isNull ) {
			...
			build.push({
				...
				data: banner( jqueryUi.pkg, null, { minify: true } ) + sqwish.minify( stripBanner( theme.css() ) )
			});
		}
