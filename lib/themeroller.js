...
// TODO Shall we use an external lib for this?
...
function ThemeRoller( vars ) {
  ...
  var themeUrl = param( Object.keys( vars ).map(function( k ) {
    return [ [ k ], [ vars[k] ] ];
  } ) );
...
ThemeRoller.prototype = {
	...
	_iconUrl: function( color ) {
		...
		this._setImage( filename, "?" + param( [ [ "new", color ], [ "w", iconDimension[0] ], [ "h", iconDimension[1] ], [ "f", "png" ], [ "fltr[]", "rcd|256" ], [ "fltr[]", "mask|icons/icons.png" ] ] ) );
	...
	_textureUrl: function( color, file, opacity ) {
		...
		this._setImage( filename, "?" + param( [ [ "new", color], [ "w", dimension[0] ], [ "h", dimension[1] ], [ "f", "png"] , [ "q", "100" ], [ "fltr[]", "over|textures/" + file + "|0|0|" + opacity ] ] ) );
