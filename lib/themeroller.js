var _ = require( "underscore" ),
	...
	fs = require( "fs" ),
	http = require( "http" ),
	logger = require( "logger" ).init( "download.jqueryui.com" );
	path = require( "path" ),
...
ThemeRoller.prototype = {
	...
	_iconUrl: function( color ) {
		...
		this._setImage( filename, "?" + querystring.stringify( { "new": color, "w": iconDimension[0], "h": iconDimension[1], "f": "png", "fltr[]": "rcd|256", "fltr[]": "mask|icons/icons.png" } ) );
	...
	css: function() {
		...
		if ( !this._css ) {
			...
			if ( this.scope ) {
				this._css = this._css.replace( /(\.ui.*)/g, this.scope + " $1" );
			}
