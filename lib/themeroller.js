...
var themeStaticCss = JqueryUi.all().reduce(function( sum, jqueryUi ) {
	sum[ jqueryUi.pkg.version ] = fs.readFileSync( jqueryUi.path + "themes/base/jquery.ui.theme.css", "utf8" );
	return sum;
}, {});
...
function ThemeRoller( options ) {
	...
	if ( version ) {
		if ( !JqueryUi.find( version ) ) {
			throw new Error( "Invalid jqueryUi version {version: \"" + version + "\"}. Fix your parameter or your add this jqueryUi to your config file." );
		} else {
			this.version = version;
		}
	}
	if ( !this.version ) {
		this.version = JqueryUi.getStable().pkg.version;
	}
